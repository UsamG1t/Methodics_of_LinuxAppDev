...
Наши патчи работают сразными контекстами, поэтому порядоких применения не так важен. но нередкой бывает работа спатчами, основанными на одном контексте, для которых порядок применения будет играть роль.
...
Возьмём те же самые патчи иприменим их к слегка изменённой программе. При этом одно изменение будет попадать в контекст, а другое - нет.
...
Как мы видим из информации патча, оба изменения применились с неточностью какого-то уровня (у нас это уровень 2). В зависимости от того, как сильно обновление изменило контекст, для неточности рассчитываетсяэтот самый уровень, и с какого-то уровня патч не применяется, как, например, в следующем примере
...
Мы уже упоминали `git` сегодня, какресурс, пришедший отчасти на замену или на подмогу методу передачи патчей между разработчиками для синхронизации работы. Однако просто выбросить механизм было бы неправильным, он очень грамотно интегрирован в работу `git`.  
