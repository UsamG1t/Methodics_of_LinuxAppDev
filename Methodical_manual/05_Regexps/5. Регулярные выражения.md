...
Регулярные выражения (Regular Expressions — `RegEx`s) описываются подклассом регулярных грамматик, называемым автоматными грамматиками. Это самый строгий из формальных языков по классификации Хомского (почитать про это можно вот [тут](https://clck.ru/3E46d6)), который описывается детерминированными и недетерминированными конечными автоматами.
