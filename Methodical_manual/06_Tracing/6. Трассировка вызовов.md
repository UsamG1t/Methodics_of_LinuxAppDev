...
exit_group(0)                           = ?
+++ exited with 0 +++
user@localhost:~> strace date |& wc # Смотрим количество 
    120     735    8213
user@localhost:~> 
```
...
```console
user@localhost:~> strace -e file date 
...
+++ exited with 0 +++
user@localhost:~> strace -e file date |& wc
     35     264    3171
user@localhost:~> 
```
...
```console
user@localhost:~> strace -z -e file date
...
+++ exited with 0 +++
user@localhost:~> strace -z -e file date |& wc
     21     111    1547
user@localhost:~> 
```
...
```console
user@localhost:~> strace -P /usr/lib/locale/en_US.utf8/LC_MESSAGES/SYS_LC_MESSAGES date
...
```console
user@localhost:~> strace -P /usr/lib/locale/en_US.utf8/LC_MESSAGES/SYS_LC_MESSAGES -v date
openat(AT_FDCWD, "/usr/lib/locale/en_US.utf8/LC_MESSAGES/SYS_LC_MESSAGES", O_RDONLY|O_CLOEXEC) = 3
...
```console
user@localhost:~> strace -P /usr/lib/locale/en_US.utf8/LC_MESSAGES/SYS_LC_MESSAGES date 2> info_not_y
Sat Oct 26 09:00:26 PM MSK 2024
user@localhost:~> strace -P /usr/lib/locale/en_US.utf8/LC_MESSAGES/SYS_LC_MESSAGES -y date 2> info_with_y
Sat Oct 26 09:00:37 PM MSK 2024

user@localhost:~> cat info_not_y | grep -E "(openat)|(fstat)"
openat(AT_FDCWD, "/usr/lib/locale/en_US.utf8/LC_MESSAGES/SYS_LC_MESSAGES", O_RDONLY|O_CLOEXEC) = 3
fstat(3, {st_mode=S_IFREG|0644, st_size=57, ...}) = 0

user@localhost:~> cat info_with_y | grep -E "(openat)|(fstat)"
openat(AT_FDCWD</home/user>, "/usr/lib/locale/en_US.utf8/LC_MESSAGES/SYS_LC_MESSAGES", O_RDONLY|O_CLOEXEC) = 3</usr/lib/locale/en_US.utf8/LC_MESSAGES/SYS_LC_MESSAGES>
...
```console
user@localhost:~> LC_ALL=en_US.utf8 strace -P /usr/lib/locale/en_US.utf8/LC_MESSAGES/SYS_LC_MESSAGES -w -C date 
openat(AT_FDCWD, "/usr/lib/locale/en_US.utf8/LC_MESSAGES/SYS_LC_MESSAGES", O_RDONLY|O_CLOEXEC) = 3
fstat(3, {st_mode=S_IFREG|0644, st_size=57, ...}) = 0
...
user@localhost:~> strace -w -c date
...
```console
user@localhost:~> ls | grep Date
user@localhost:~> strace -w -c -o Date_exec_wall date
Sat Oct 26 09:12:11 PM MSK 2024
user@localhost:~> ls | grep Date
Date_exec_wall
user@localhost:~> cat Date_exec_wall 
% time     seconds  usecs/call     calls    errors syscall
------ ----------- ----------- --------- --------- ----------------
...
```console
user@localhost:~> strace -P /usr/lib/locale/en_US.utf8/LC_MESSAGES/SYS_LC_MESSAGES -k date 
openat(AT_FDCWD, "/usr/lib/locale/en_US.utf8/LC_MESSAGES/SYS_LC_MESSAGES", O_RDONLY|O_CLOEXEC) = 3
...
user@localhost:~> strace -e read -s100 cat Hello.txt 
...
user@localhost:~> strace -P /dev/null -e openat cat /dev/null{,,}
...
```console
user@localhost:~> strace -P /dev/null -e openat -e fault=openat:error=EACCES cat /dev/null{,,}
...
```console
user@localhost:~> strace -P /dev/null -e openat --fault=openat:error=EACCES:when=3 cat /dev/null{,,}
openat(AT_FDCWD, "/dev/null", O_RDONLY) = 3
openat(AT_FDCWD, "/dev/null", O_RDONLY) = 3
openat(AT_FDCWD, "/dev/null", O_RDONLY) = -1 EACCES (Permission denied) (INJECTED)
cat: /dev/null: Permission denied
...
user@localhost:~> strace -P /dev/null -e openat -e fault=openat:error=EACCES:when=2 cat /dev/null{,,}
openat(AT_FDCWD, "/dev/null", O_RDONLY) = 3
openat(AT_FDCWD, "/dev/null", O_RDONLY) = -1 EACCES (Permission denied) (INJECTED)
cat: /dev/null: Permission denied
openat(AT_FDCWD, "/dev/null", O_RDONLY) = 3
...
user@localhost:~> strace -P /dev/null -e openat --inject=openat:error=EACCES:when=2:signal=SIGINT cat /dev/null{,,}
openat(AT_FDCWD, "/dev/null", O_RDONLY) = 3
openat(AT_FDCWD, "/dev/null", O_RDONLY) = -1 EACCES (Permission denied) (INJECTED)
...
```console
user@localhost:~/LinuxAppDev/06_Tracing/Lection> cc rand.c -o rand
user@localhost:~/LinuxAppDev/06_Tracing/Lection> ./rand 
./rand: 1804289383, 1
./rand: 846930886, 3
./rand: 1681692777, 5
./rand: 1714636915, 7
user@localhost:~/LinuxAppDev/06_Tracing/Lection> ./rand q w e
e: 1804289383, 1
e: 846930886, 3
e: 1681692777, 5
e: 1714636915, 7
user@localhost:~/LinuxAppDev/06_Tracing/Lection> 
```
...
```console
user@localhost:~/LinuxAppDev/06_Tracing/Lection> ldd rand
...
```console
user@localhost:~/LinuxAppDev/06_Tracing/Lection> cc -shared -fPIC ura.c -o ura.so
user@localhost:~/LinuxAppDev/06_Tracing/Lection> LD_PRELOAD=`pwd`/ura.so ./rand 
./rand: 42, 1
./rand: 42, 3
./rand: 42, 5
./rand: 42, 7
