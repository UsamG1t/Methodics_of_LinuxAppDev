...
Однако, в будущем мы так поступать не хотим — здесь ведётся трансляция всех файлов сразу. Если бы их было сильно дольше, мы бы, как минимум, проигрывали в скорости компиляции.
...
**Генерат** — файл, отсутствующий в исходных текстах, но появляющийся в процессе сборки. **Целевой генерат** — генерат, ради которого была затеяна сборка (например, исполняемый файл или библиотека). **Промежуточный генерат** — для чего-то нужен в процессе сборки, но по её завершении не имеет применения (например, объектные файлы из состава библиотеки).
...
```
Теперь компоновкой исполняемого файла и сборкой объектного файла из исходного заведуют встроенное в `Makе` правила (они, например, добавляют CFLAGS к каждой компиляции):
...
Начнём со статической библиотеки. По сути своей, она представляет собой объектные файлы, сложенные архиватором в единый файл. Так как исполняемым модулем проекта у нас является только prog.o,  другие два объектника выступают в роли библиотечных, объединим их в библиотеку и соберём проект с ней:
...
Аналогичных мейку систем сборки много, самый «модный» — [Ninja](https://ninja-build.org). Но вручную `ninja`-фалы не пишет никто; он специально разработан, чтобы обрабатывать генераты более высокоуровневых систем, о котоых разговор пойдёт позже.
