...
1. Для второго примера проекта из сегодняшнего раздела написать `Makefile` так, чтобы в цели по умолчанию:        
    ...
    3. Имелась цель `test:`, для достижения которой:
		a. каждый из трёх бинарников запускается
		...
		b. Затем _выводы_ соответствующих запусков сравниваются (например, с помощью `cmp file1 file2`, который вернёт ненулевой статус ошибки, если файлы не одинаковы)
