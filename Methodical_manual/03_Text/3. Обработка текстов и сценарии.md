...
В качестве выражения условия у нас используется результат работы операций над данными или результат зfвершения (корректного или нет) запускаемой программы.
```console
